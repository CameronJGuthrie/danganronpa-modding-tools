 ...
 */
function parseEntryV3(buffer: Buffer, offset: number): GxtEntry {
  return {
 ...
 */
function vitaUnswizzle(
  width: number,
  height: number,
  isBlockEncoding = false,
): (pointCount: number) => { x: number; y: number } {
  // Build the bit field for coordinate transformation
  const bitField: Array<[number, number]> = [];
  ...
  return function transform(pointCount: number): { x: number; y: number } {
 ...
 */
function decodeI8(buffer: Buffer, entry: GxtEntry, palette: Rgba[]): DecodedTexture {
 ...
 */
function decodeI4(buffer: Buffer, entry: GxtEntry, palette: Rgba[]): DecodedTexture {
  const width = entry.width;
  const height = entry.height;
  const pixelData = new Uint8Array(width * height * 4);
 ...
 */
async function savePNG(pixelData: Uint8Array, width: number, height: number, outputPath: string): Promise<void> {
...
/** Convert a GXT file to one PNG per texture, returning the paths written. */
export async function convertGXT(inputPath: string, outputDir: string, silent = false): Promise<string[]> {
...
async function main(): Promise<void> {
  ...
  if (args.length < 1) {
    console.log(`Usage: gxt-to-png.ts <input.gxt> [output-dir]
...
Example:
  node gxt-to-png.ts texture.gxt ./output/`);
    process.exit(1);
  }
