...
class FileTypeChecker {
  ...
  static isPAK(data: Buffer): boolean {
  ...
  static isSPFT(data: Buffer): boolean {
    if (data.length < 4) return false;
  ...
  static isTGA(data: Buffer): boolean {
  ...
  static isGXT(data: Buffer): boolean {
    if (data.length < 12) return false;
...
class PakEntry {
  ...
  offset: number;
  size: number;
  /** Populated only while rewriting an archive in place (see replaceTextEntry). */
  content?: Buffer;
...
interface ReadPakResult {
  pak: Pak;
  buffer: Buffer;
}

async function readPak(filePath: string): Promise<ReadPakResult> {
...
/** A file to pack: its path on disk paired with the name it takes inside the archive. */
type InputFile = [filePath: string, entryName: string];

async function writePak(pak: Pak, outputPath: string, inputFiles: InputFile[]): Promise<void> {
...
async function extractFiles(inputPath: string, outputPath: string, silent: boolean): Promise<void> {
  await extractPak(inputPath, outputPath, silent);
}

async function packFiles(inputDirs: string[], outputPath: string, silent: boolean): Promise<void> {
  ...
  for (const [filePath] of inputFiles) {
    const fileStats = await stat(filePath);
    const entryIndex = Number.parseInt(basename(filePath), 10);
...
async function replaceTextEntry(inputPath: string, index: string, content: string): Promise<void> {
  const { pak, buffer } = await readPak(inputPath);
...
function showUsage(): void {
  console.log(`Usage: pak-archiver.ts [options] <command> [args...]
...
Examples:
  node pak-archiver.ts list archive.pak
  node pak-archiver.ts extract archive.pak output/
  node pak-archiver.ts create input/ archive.pak
  node pak-archiver.ts replace archive.pak 5 "New text"`);
}
