...
export function mapProperty<N extends string | number, T extends string>(
  ...
  return Object.fromEntries(
    Object.entries(input).map(([k, v]) => [k, { name: (v as Record<T, string>)[key] }]),
...
export function flatMapProperty<N extends string | number, T extends string>(
  ...
  return Object.fromEntries(Object.entries(input).map(([k, v]) => [k, (v as Record<T, string>)[key]])) as Record<
    N,
    string
  >;
}
